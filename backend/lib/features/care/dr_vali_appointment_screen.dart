...
class DrValiAppointmentScreen extends StatefulWidget {
  ...
  @override
  State<DrValiAppointmentScreen> createState() => _DrValiAppointmentScreenState();
}

class _DrValiAppointmentScreenState extends State<DrValiAppointmentScreen> {
  ...
  List<String> _generateRandomSlotsForDay(DateTime day) {
    final rng = Random();
    final slotCount = rng.nextInt(3) + 3; // 3–5 slots
    final baseTimes = [8, 9, 10, 11, 13, 14, 15, 16];
    baseTimes.shuffle();
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          ...
          '${formatPrettyDate(_selectedDay!)} at $slot with Dr. Vali?',
        ),
        actions: [
          TextButton(
